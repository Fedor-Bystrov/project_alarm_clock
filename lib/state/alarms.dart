...
class AlarmsState extends ChangeNotifier {
  ...
  Future<void> updateAlarm(Alarm initial, Alarm updated) async {
    _alarms.remove(initial);
    _alarms.add(updated);
    await persistAndNotify();
    await _alarmManager.addAlarm(updated);
  }
  ...
  Future<void> switchAlarm(Alarm alarm, bool value) async { // TODO disable alarm job
    alarm.enabled = value;
    await persistAndNotify();
  }
